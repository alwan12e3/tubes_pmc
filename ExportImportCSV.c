...
void savetocsvasisten(jadwal_t [15][5]) {
    /*Membuat file agar hasil dapat dimasukkan ke dalam file csv*/
    FILE *outfile;
    outfile = fopen("Tubes_Jadwal Asisten.csv","w");
    ...
    for (int minggu=3; minggu<=15; minggu++) {
        for (int hari=0; hari<=4; hari++) {
            ...
            /*LAB 1 = lab[r=0]*/
            fprintf(outfile,"%s", j[minggu][hari].lab[0].assist);
            /*LAB 2 = lab[r=1]*/
            fprintf(outfile,"%s", j[minggu][hari].lab[1].assist);
            /*LAB 3 = lab[r=2]*/
            fprintf(outfile,"%s", j[minggu][hari].lab[2].assist);
            /*LAB LSS = lab[r=3]*/
            fprintf(outfile,"%s\n", j[minggu][hari].lab[3].assist);
...
void importcsv() {
    ...
    char *file;
    //input user
    printf("Masukkan nama file, tambahkan .csv: "); scanf("%s", file);
    //belum beres hehe
    FILE* inp;
    inp = fopen(file,"r");
    if (inp == NULL) {
